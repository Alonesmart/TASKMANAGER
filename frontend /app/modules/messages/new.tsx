...
function ContactAvatar({
  ...
  return (
    ...
    >
      <Text style={[styles.contactAvatarText, { color: bg, fontSize: size * 0.35 }]}>
        {initials}
      </Text>
    </View>
  );
...
function RecipientChip({
  ...
  return (
    <View style={styles.chip}>
      ...
      <Text style={styles.chipText}>{contact.nom.split(" ")[0]}</Text>
...
export default function NewMessageScreen() {
  ...
  const toggleRecipient = (contact: User) => {
    setRecipients((prev) =>
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      await messageService.sendMessage({
        contenu: body,
        type_conversation: recipients.length > 1 ? 'groupe' : 'direct',
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerCenter}>
          ...
          : recipients.map((r) => r.nom.split(" ")[0]).join(", ")}
          </Text>
        </View>
        {step === "recipients" ? (
          <TouchableOpacity
            ...
            <View style={styles.toRow}>
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ flex: 1 }}>
                <View style={{ flexDirection: "row", gap: 6 }}>
                  {recipients.map((r) => {
                    const seed = r.id % 6;
                    const contactColors = ["#25D366","#128C7E","#075E54","#34B7F1","#ECE5DD","#DCF8C6"];
                    const dotColor = contactColors[seed];
                    return (
                      <View key={r.id} style={styles.toChip}>
                        <View style={[styles.toChipDot, { backgroundColor: dotColor }]} />
                        <Text style={styles.toChipText}>{r.nom}</Text>
                      </View>
                    );
                  })}
                </View>
