...
export default function NouveauProjetScreen() {
  ...
  const handleIARediger = async () => {
    if (!nom.trim()) {
      Alert.alert(t("common.error"), "Veuillez d'abord saisir un nom de projet.");
      return;
    }
    setLoadingIA(true);
    try {
      const res = await aiService.suggestDescription(nom, 'projet');
      setDescription(res.description);
    } catch (error) {
      console.error("Error generating project IA description:", error);
