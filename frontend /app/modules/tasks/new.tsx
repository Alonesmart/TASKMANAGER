...
export default function NouvelleTacheScreen() {
  ...
  const [loadingRediger, setLoadingRediger] = useState(false);
  const [loadingPrio, setLoadingPrio]       = useState(false);

  const handleIARediger = async () => {
    if (!titre.trim()) {
      Alert.alert(t("common.error"), "Veuillez d'abord saisir un titre.");
      return;
    }
    setLoadingRediger(true);
    try {
      const res = await aiService.suggestDescription(titre, 'tache');
      setDescription(res.description);
    } catch (error) {
      console.error("Error generating IA description:", error);
      Alert.alert(t("common.error"), "Impossible de générer la description.");
    } finally {
      setLoadingRediger(false);
    }
  };

  const handleIASuggererPriorite = async () => {
    if (!titre.trim()) {
      Alert.alert(t("common.error"), "Veuillez d'abord saisir un titre.");
      return;
    }
    ...
    try {
      const res = await aiService.suggestPriority(
        titre,
        description,
        dateFin ? dateFin.toISOString() : undefined
      );
      const suggested: 'faible' | 'moyenne' | 'haute' = res.priorite as any;
      setPriorite(suggested);
      Alert.alert("Suggestion IA", `Priorité suggérée : ${suggested.toUpperCase()}`);
    } catch (error) {
      console.error("Error fetching suggested priority:", error);
      Alert.alert(t("common.error"), "Impossible de suggérer la priorité.");
    } finally {
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      <ScrollView style={styles.body} showsVerticalScrollIndicator={false}>
        ...
        <TouchableOpacity
          style={styles.iaBtn}
          onPress={handleIARediger}
          disabled={loadingRediger}
          activeOpacity={0.8}
        >
          {loadingRediger ? (
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
              ...
              <Text style={styles.iaBtnText}>Suggérer la description (IA)</Text>
            </View>
          )}
        </TouchableOpacity>
        ...
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
          <Text style={[styles.label, { marginBottom: 0 }]}>{t("new_tasks.priority_label")}</Text>
          <TouchableOpacity
            style={styles.prioIabtn}
            onPress={handleIASuggererPriorite}
          ...
          >
            {loadingPrio ? (
              ...
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 4 }}>
                ...
                <Text style={styles.prioIaText}>Suggérer par l'IA</Text>
              </View>
            )}
          </TouchableOpacity>
        </View>
